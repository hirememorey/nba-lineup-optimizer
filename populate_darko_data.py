#!/usr/bin/env python3
"""
Populate DARKO data into the NBA Lineup Optimizer database.

This script processes the DARKO data from nbarapm.com and populates the PlayerSkills table
with offensive and defensive skill ratings for the 2022-23 season (and optionally other seasons).

Usage:
    python populate_darko_data.py [--season SEASON] [--all-seasons]
"""
...
def populate_darko_data(season=None, all_seasons=False):
    ...
    print("🏀 NBA Lineup Optimizer - DARKO Data Population")
    print("=" * 50)
    ...
    if all_seasons:
        ...
        # Map season format: 2022-23 -> 2023 in DARKO data
        if season == "2022-23":
            darko_season = 2023
        elif season == "2023-24":
            darko_season = 2024
            ...
            darko_season = int(season.split('-')[1])  # Extract second year
...
def main():
    parser = argparse.ArgumentParser(description='Populate DARKO data into NBA Lineup Optimizer database')
    parser.add_argument('--season', help='Season to process (e.g., 2022-23, 2023-24, 2024-25)')
